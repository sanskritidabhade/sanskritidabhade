# Hi there, I'm Sanskriti! 👋

### Honours Computer Science Student | Math Minor | Tech Entrepreneur

I am a CS student at Lakehead University with a focus on Business (+ Certificate of Entrepreneurship) and Mathematics. I bridge the gap between technical execution and product strategy—whether I'm lead-developing a startup or optimizing algorithms for competitive programming.

---

## 🚀 Current Focus
- **Founder @ [Spot2Go.app](https://spot2go.app):** Building a B2B platform to streamline how people discover and interact with local cafes.
- **Game Development:** Currently troubleshooting networking and AI behavior for an unannounced title.
- **Academics:** Maintaining a CGPA of 84.60 and exploring the intersections of Linear Algebra and AI.

## 🛠 Tech Stack
- **Languages:** Java, Python, C++, JavaScript, C
- **Frontend/Web:** React, HTML5, CSS3
- **Tools & Platforms:** Git, GitHub Actions, Node.js
- **Business/Leadership:** Project Management, Sales Workflow, Team Leadership (7+ members)

## 🏆 Highlights
- **EcoThread:** Led a 7-person team to the **Top 5 Finals** in a major hackathon.
- **Competitive Programming:** Placed **Top 2** in the GOPS Coding Contest (March 2025).
- **Dean's List:** Recognized for academic excellence at Lakehead University.

---

## 📫 Let's Connect
- **LinkedIn:** [linkedin.com/in/sanskritidabhade](#) - **Portfolio/Startup:** [spot2go.app](https://spot2go.app)
- **Email:** [sanskritidabhade@gmail.com]

*"Ideas are cheap. Execution is expensive. I’m here to collect."*
